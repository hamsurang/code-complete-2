---
sidebar_position: 2
title: "Week 2 — 31~34장: 레이아웃 · 자기 설명 코드 · 개발자 성격 · 장인정신"
...
function calcTotal(order?: Order): number {
  ...
  const discounted =
    order.coupon.type === "rate"
      ? subtotal * (1 - order.coupon.value)
...
type Order = { id: string; storeId: string; status: OrderStatus; total: number; paid?: number; memo?: string; customerPhone?: string };
...
export async function act(deps: Deps, input: {
  user: User; orderId: string;
  action: "cancel" | "refund" | "memo" | "forcePaid";
  memo?: string; paymentId?: string; amount?: number; notify?: boolean;
}) {
  ...
  if (input.action === "memo") {
    ...
    o.memo = input.memo; await deps.saveOrder(o);
  ...
  if (input.action === "forcePaid") {
    ...
    o.status = "paid"; o.paid = o.total; await deps.saveOrder(o);
  ...
  if (input.action === "cancel") {
    ...
    o.status = "canceled"; await deps.saveOrder(o);
  ...
  if (input.action === "refund") {
    ...
    if (!pg.ok) { await deps.audit({ t: "refund_fail", orderId: o.id, code: pg.code }); return { ok: false, reason: "pg_fail", code: pg.code }; }
    o.status = "refunded"; await deps.saveOrder(o);
...
type ActionResult =
  | { ok: true; order: Order; refunded?: number }
...
class OrderActionError extends Error {
  constructor(public readonly reason: FailReason, public readonly code?: string) {
    super(reason);
  }
}
...
async function handleMemo(
  deps: Deps, user: User, order: Order, memo: string
): Promise<Order> {
  order.memo = memo;
...
async function handleForcePaid(
  deps: Deps, user: User, order: Order
): Promise<Order> {
  assertOwner(user);
...
async function handleCancel(
  deps: Deps, user: User, order: Order, notify: boolean
): Promise<Order> {
...
async function handleRefund(
  deps: Deps, user: User, order: Order,
  paymentId: string, amount?: number, notify?: boolean
    ...
    switch (input.action) {
      ...
      case "refund": {
        const { order: updated, refunded } = await handleRefund(
          deps, user, order, input.paymentId, input.amount, input.notify
        );
